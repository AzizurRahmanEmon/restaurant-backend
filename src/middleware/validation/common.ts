...
export const validateIdParam = (
  ...
  const paramValue = id || customerId;
  ...
  // Ensure paramValue is a string (req.params can be string | string[])
  const idToValidate = Array.isArray(paramValue) ? paramValue[0] : paramValue;
...
export const validateParamSafety = (paramName: string) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    // Ensure value is a string (req.params can be string | string[])
    ...
    req.params[paramName] = sanitizeInput(paramValue);
    next();
  };
};
